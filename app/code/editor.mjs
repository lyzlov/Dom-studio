...
import { buildSite, imageBases } from '../../code/assemble.mjs';
import { setMarkup, parseSet, replaceTemplate } from '../../code/template.mjs';
import { setLang, setWord } from '../../code/lang.mjs';
import { splitCaptions, mergeCaptions, captionFields } from '../../code/captions.mjs';
...
const СКРИПТ_ИСХОДНИК = 'code/script.src.js';
const СКРИПТ = 'code/script.js';
...
export const S = {
  ...
  texts: new Map(),         // data/text/**.html → содержимое
...
function sourcesHelp() {
  ...
  строки.push(['locale', `app/lang/${lang()}/ui.json`, false]);
...
function fileSection(путь) {
  ...
  if (путь.startsWith('data/')) return t('files.structure', 'Site structure');
  if (путь.startsWith('code/')) return t('files.elements');
  if (путь.startsWith('assets/')) return t('files.design');
  return null;
}
 ...
 * `code/names.<язык>.json`, имена оформления — в манифесте. Английского
...
/* Значки, которыми пользуется редактор. Все — файлы из assets/icons, все
