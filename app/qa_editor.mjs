 ...
 * По умолчанию http://127.0.0.1:8099/app/
 */

import { chromium } from 'playwright';
import { readFileSync } from 'fs';
import { parseSet } from '../code/template.mjs';
import { parseMarkup, serializeMarkup } from './code/markup.mjs';
import { fileURLToPath } from 'url';
import { dirname, join } from 'path';

const href = process.argv[2] || 'http://127.0.0.1:8099/app/';
...
function checkCSS() {
  const текст = readFileSync(join(здесь, 'assets/style.css'), 'utf8').replace(/\/\*[\s\S]*?\*\//g, '');
  текст.split('\n').forEach((строка, i) => {
...
function checkMarkup() {
  const set = parseSet(readFileSync(join(здесь, '../code/markup.html'), 'utf8'));
