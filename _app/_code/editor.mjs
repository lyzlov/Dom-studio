...
const СКРИПТ_ИСХОДНИК = '_code/script.src.js';
const СКРИПТ = '_code/script.js';
...
export const S = {
  ...
  texts: new Map(),         // _data/text/**.html → содержимое
...
/**
 * Путь до слов: у файла данных они лежат под тем же именем в `_lang/<язык>/`.
 * Слова, которые произносит сам код, файла данных не имеют — им отведено имя
 * `ui`, и это единственное исключение из зеркала.
 */
const путьСлов = имя => String(FILES().words || '_lang/{lang}/{name}.json')
...
function sourcesHelp() {
  ...
  строки.push(['locale', `_app/_lang/${lang()}/ui.json`, false]);
 ...
 * `_code/names.<язык>.json`, имена оформления — в манифесте. Английского
...
/* Значки, которыми пользуется редактор. Все — файлы из _assets/icons, все
