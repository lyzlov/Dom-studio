...
export const ВИДЫ = {
  ...
  пост: (п, ctx) => карточка({
    ссылкаНа: ссылка(ctx.путь, `blog/${п.id}/index.html`), заголовок: п.заголовок,
...
const контактыБлока = (б, ctx) => {
  ...
  const строки = [
    ...
    ['Telegram', `<a href="https://${esc(к.telegram)}" target="_blank" rel="noopener">${esc(к.telegram)}</a>`],
  ];
  const сетка = `<div class="contact-grid">\n`
    + строки.map(([п, з]) => `<div class="contact-item"><span class="contact-label">${esc(п)}</span>${з}</div>`).join('\n')
    + `\n</div>`;
  if (!б.карта) return сетка;
  // Карта грузится отложенно: тяжёлый чужой виджет не должен задерживать страницу.
  const [ш, д] = к.координаты;
  return сетка + `
<div class="map-block">
<iframe src="https://yandex.ru/map-widget/v1/?ll=${д}%2C${ш}&z=17&pt=${д},${ш},pm2rdm" title="Карта: ${esc(к.место)}, ${esc(к.адрес)}" loading="lazy" allowfullscreen></iframe>
</div>
<p class="map-actions"><a class="btn" href="${к.маршрут}" target="_blank" rel="noopener">Маршрут в Яндекс.Картах</a></p>`;
};
...
export const СТРАНИЧНЫЕ = { вкладки: вкладкиБлока };
/* #endregion */
